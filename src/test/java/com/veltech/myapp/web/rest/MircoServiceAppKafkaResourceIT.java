package com.veltech.myapp.web.rest;
...
import com.veltech.myapp.IntegrationTest;
import com.veltech.myapp.config.EmbeddedKafka;
import com.veltech.myapp.config.KafkaSseConsumer;
import com.veltech.myapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class MircoServiceAppKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/mirco-service-app-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/mirco-service-app-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/mirco-service-app-kafka/unregister"));
                return;
            }
        }
